...
export class ConfigRefresher {
  constructor({configStorage, refreshHandler} = {}) {
    ...
    // schedule refresh job if refresh is enabled
    const {refresh: {enabled}} = bedrock.config['service-core'].configStorage;
    if(enabled) {
      this._schedule();
    }
  }
  ...
  async _markRecord({record}) {
    ...
    const now = Date.now();
    const after = now + isolateTimeout;
    const collection = this.configStorage._getCollection();
    ...
    if(result.modifiedCount === 1) {
      record.meta.refresh.after = after;
  ...
  async _start() {
    ...
    while(!signal.aborted) {
      try {
        ...
        // updates from concurrent processes (but do not fuzz *if* interval
        // is already less than 5 minutes to allow for systems where more
        // rapid refresh is configured)
        const fuzz = interval < FIVE_MINUTES ? 0 : FIVE_MINUTES;
        await rangeDelay(interval, interval + fuzz, {signal});
      } catch(error) {
  ...
  async _refreshOne() {
    ...
    try {
      ...
      if(marked) {
        ...
        await this.refreshHandler({record, signal});
      }
    } catch(error) {
