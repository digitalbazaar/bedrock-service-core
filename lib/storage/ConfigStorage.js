...
export class ConfigStorage {
   ...
   */
  constructor({serviceType, storageCost} = {}) {
    assert.string(serviceType, 'serviceType');
    assert.object(storageCost, 'storageCost');
    const collectionName = `service-object-config-${serviceType}`;
  ...
  /**
   * Establishes a new service object instance by inserting its configuration
   * into storage.
   *
   * @param {object} options - The options to use.
   * @param {object} options.config - The instance configuration.
   *
   ...
   *
   * @param {object} options - The options to use.
   * @param {string} options.controller - The controller for the instances to
   *   retrieve.
   ...
   */
  async find({controller, query = {}, options = {}} = {}) {
    // force controller ID
   ...
   *
   * @param {object} options - The options to use.
   * @param {string} options.id - The ID of the instance.
   *
   ...
   */
  async getUsage({meterId} = {}) {
    // count all configs with the given meter ID
    const count = await _getCollection().countDocuments({
      'config.meterId': meterId
    });
    const {storageCost} = this;
    const usage = {storage: count};
    return usage;
  };
