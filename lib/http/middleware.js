...
const {
  helpers: {inspectCapabilityChain: _inspectCapabilityChain}
} = brZCapStorage;
...
async function inspectCapabilityChain({capabilityChain, capabilityChainMeta}) {
  // check expiration status of each capability in the chain and log if
  // configured; this runs before the revocation check so that expiration
  // issues are logged even if the zcap is also revoked
  _logCapabilityChainExpiration({capabilityChain});

  // delegate to the original inspector for revocation checking
  return _inspectCapabilityChain({capabilityChain, capabilityChainMeta});
}
...
function _logCapabilityChainExpiration({capabilityChain}) {
  const {zcapExpiration} = bedrock.config['service-core'];
