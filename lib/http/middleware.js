...
import {validateAccessToken} from '../oauth2.js';
...
// checks OAuth2 JWT access token against `oauth2` config expected values
export function authorizeOAuth2AccessToken({
  getExpectedValues, getRootController
} = {}) {
  return asyncHandler(async function _getConfig(req, res, next) {
    // code assumes authorization header has already been checked to exist and
    // value starts with `Bearer`
    const header = req.get('authorization');
    const token = header.match(OAUTH2_TOKEN_REGEX)[0];
    ...
    if(!issuerConfigUrl) {
      // FIXME: throw error, oauth2 not supported by this service object
    }

    // FIXME: call `getExpectedValues`
    // FIXME: default `target` is `https://${expected.host}${req.originalUrl}`;
    // FIXME: default `action` is...
    /*
    ...
    */
    // FIXME: combine `action` and `target` to create expected scope(s)
    // FIXME: set expected audience to expected `host`
    // FIXME: prepare all params to pass to JWT verification method
    // FIXME: set required `aud` claim
    const {error} = await validateAccessToken(
      {issuerConfigUrl, token, claims, scopes});
    if(error) {
      // FIXME: throw or send OAuth2 error via res?
      throw error;
    }
...
function _useDetectedAuthzMethod({authzMiddleware}) {
  return function useDetectedAuthzMethod(req, res, next) {
    ...
    const oauth2 = req.get('authorization')?.startsWith('Bearer ') &&
      // config MUST have `oauthIssuerBaseUrl` or OAuth2 is not supported
      // by the service object instance
      req.serviceObject.config.oauthIssuerBaseUrl;
    if(zcap && oauth2) {
