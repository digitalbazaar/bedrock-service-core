...
// FIXME: rework authz middle functions to so that they handle both zcap and /
// or oauth2 with the same helpers (`getExpectedValues` and
// `getRootController`)
...
export function authorizeInvocation({
  ...
  const {
    authorization, authorizeZcapInvocationOptions
  } = bedrock.config['service-core'];

  // FIXME: add middleware to get request authz type; throw if more than one
  // type is used throw error; if none are present, throw authz needed error

  // FIXME: add middleware for matching authz type
}

// FIXME: remove `export` from specific authz types; move `authorizeConfigX`
// export function up
...
export function authorizeZcapInvocation({
  ...
  if(!authorization.zcap) {
    // zcap-authz disabled, go to next middleware
    return _nextMiddleware;
  }
...
// creates middleware for service object config route oauth2 authz checks
export function authorizeConfigOAuth2AccessToken({expectedAction} = {}) {
  return _createConfigAuthzMiddleware({
    authzFn: authorizeOAuth2AccessToken,
    expectedAction
  });
}

// creates middleware for service object config route zcap authz checks
export function authorizeConfigZcapInvocation({expectedAction} = {}) {
  return _createConfigAuthzMiddleware({
    authzFn: authorizeZcapInvocation,
    expectedAction
  });
}
...
function _createConfigAuthzMiddleware({authzFn, expectedAction} = {}) {
  return authzFn({
...
function _nextMiddleware(req, res, next) {
  next();
}
