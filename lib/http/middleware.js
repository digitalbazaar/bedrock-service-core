...
import {checkOAuth2ExpectedValues, validateAccessToken} from '../oauth2.js';
...
function _authorizeOAuth2AccessToken({getExpectedValues}) {
  return asyncHandler(async function _getConfig(req, res, next) {
    ...
    try {
      ...
      checkOAuth2ExpectedValues({req, expected});
    } catch(error) {
      onError({error});
    }

    // set expected defaults
    expected.action = expected.action ?? DEFAULT_ACTION_FOR_METHOD.get(req.method);
