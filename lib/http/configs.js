...
export function addRoutes({app, service}) {
  ...
  app.post(
    ...
    asyncHandler(async (req, res, next) => {
      ...
      try {
        ...
        // get any HTTP-returned error data
        const error = e.data || e;
        // expose one level of causation only
        if(error.cause) {
          error.cause = _createPublicError({error: error.cause});
        }
        // surface lower-level errors during instance creation
        const cause = _createPublicError({error});
        throw new BedrockError(
          `Meter usage for meter "${meterId}" could not be checked.`, {
            name: error.name ?? 'OperationError',
            details: {
              httpStatusCode: error.details?.httpStatusCode ?? 400,
              public: true
            },
            cause
          });
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      if(validateConfigFn) {
        const {valid, error} = await validateConfigFn({
          req, config, op: 'update', existingConfig
        });
        if(!valid) {
          throw error;
        }
...
function _createPublicError({error}) {
  return new BedrockError(error.message, {
    name: error.name ?? 'OperationError',
    details: {httpStatusCode: 400, public: true, ...error.details},
    cause: error.cause
  });
}
