...
cfg.configStorage = {
  refresh: {
    // when enabled, refresh jobs will be scheduled for any service created
    // with a `refreshHandler`; otherwise no refresh jobs will be scheduled
    enabled: true,
    // schedule a refresh every 5 minutes; may be slightly randomized
    // default: 5 minutes
    interval: 5 * 60 * 1000,
    // maximum time to allow for a config record to be isolated for refresh
    // default: 10 minutes
    isolateTimeout: 10 * 60 * 1000
  }
};

// zcap expiration logging configuration for observability / monitoring alerts
cfg.logging = {};
cfg.logging.zcapExpiration = {
  // log name used for filtering/alerting
  // example filter: { $.logName = "zcap-expiration" }
  logName: 'zcap-expiration',
  // log events for zcaps that are near expiration (not yet expired, but within
  // the threshold); set to `false` to disable near-expiration logging
  logNearExpiration: {
    // threshold in milliseconds; zcaps expiring within this window will be
    // logged; default: 7 days
    threshold: 7 * 24 * 60 * 60 * 1000
  },
  // log events for zcaps that have already expired; set to `false` to disable
  // expired zcap logging
  logExpired: false
};
