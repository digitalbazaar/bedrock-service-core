 ...
 * @param {Array} [options.zcapReferenceIds] - Any expected zcap reference IDs
 *   that must be present in configs.
...
export async function createService({
  ...
  if(zcapReferenceIds !== undefined) {
    ...
    zcapReferenceIds.every(id => assert.string(id));

    // create `zcaps` schema specific to service
    const zcaps = bedrock.util.clone(_zcaps);
    for(const referenceId of zcapReferenceIds) {
