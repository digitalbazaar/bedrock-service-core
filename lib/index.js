 ...
 * @param {Function} [options.refreshHandler] - A function that will be passed
 *   a configuration record to "refresh" it; the implementation is expected to
 *   update the record and set its `meta.refresh` properties according to the
 *   service requirements (an example of a refresh handler might be one that
 *   refreshes the zcaps stored in the configuration object in the record); a
 *   refresh job will be scheduled if and only if a `refreshHandler` is given.
...
export async function createService({
  ...
  cacheConfig, refreshHandler, usageAggregator,
  validation: {
  ...
  const configStorage = new ConfigStorage({
    serviceType, storageCost, cacheConfig, refreshHandler
  });
