...
const SUPPORTED_AUTHZ_METHODS = ['zcap', 'oauth2'];
...
bedrock.events.on('bedrock.init', () => {
  ...
  let enabledCount = 0;
  for(const k of keys) {
    if(!SUPPORTED_AUTHZ_METHODS.includes(k)) {
      throw new Error(`Authorization method "${k}" not supported.`);
    }
    if(authorization[k]) {
      enabledCount++;
    }
  ...
  if(enabledCount === 0) {
    throw new Error('At least one authorization method must be enabled.');
  }
});
...
export async function createService({
  ...
  if(zcapReferenceIds !== undefined) {
    ...
    const zcaps = klona(schemas.zcaps);
    ...
    createConfigBody = klona(createConfigBody);
    updateConfigBody = klona(updateConfigBody);
