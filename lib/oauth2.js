...
export async function validateAccessToken({
  ...
  const {payload, protectedHeader} = await jwtVerify(jwt, jwkSet, {algorithms});
...
async function _getUncachedIssuerConfig({issuerConfigUrl}) {
  ...
  The `host` and `path` element must be parsed from `issuer` and checked
  against `issuerConfigUrl` like so:

  For issuer `<host>` (no path), `issuerConfigUrl` must match:
  `<host>/.well-known/<any-path-segment>`

  For issuer `<host><path>`, `issuerConfigUrl` must be:
  `<host>/.well-known/<any-path-segment><path>` */
  const {pathname: wellKnownPath} = new URL(issuerConfigUrl);
  const anyPathSegment = wellKnownPath.match(WELL_KNOWN_REGEX)[1];
  const {host, pathname} = new URL(issuer);
  let expectedConfigUrl = `${host}/.well-known/${anyPathSegment}`;
  if(pathname !== '/') {
